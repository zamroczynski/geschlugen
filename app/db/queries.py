sql = {
    ...
    "getUser": "SELECT users.id as id, users.login as login, users.password as password role.name as role FROM users "
               "WHERE login={login} INNER JOIN role ON users.role = role.id",
}
